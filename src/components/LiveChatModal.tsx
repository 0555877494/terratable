      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-24 left-6 z-[95] w-80 sm:w-96 bg-white dark:bg-stone-800 rounded-2xl shadow-2xl border-2 border-stone-200 dark:border-stone-700 overflow-hidden"
          >
            {/* Header */}
            ...
            {/* Messages */}
            <div className="h-80 overflow-y-auto p-4 space-y-3 bg-stone-50 dark:bg-stone-900">
                ...
                >
                  <div className={`flex items-start gap-2 max-w-[80%] ${message.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                      message.sender === 'user' ? 'gradient-bg' : 'bg-stone-200 dark:bg-stone-700'
                    }`}>
                      {message.sender === 'user' ? (
                        <User className="w-4 h-4 text-white" />
                      ) : (
                        <Bot className="w-4 h-4 text-stone-600 dark:text-stone-400" />
                      )}
                    </div>
                    <div className={`px-4 py-2.5 rounded-2xl text-sm ${
                      message.sender === 'user'
                        ? 'gradient-bg text-white rounded-tr-sm'
                        : 'bg-white dark:bg-stone-800 border-2 border-stone-200 dark:border-stone-700 text-stone-700 dark:text-stone-300 rounded-tl-sm'
                    }`}>
                      {message.text}
                    </div>
                  </div>
            ...
            {/* Input */}
            <div className="p-4 border-t-2 border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl border-2 border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-200 focus:border-amber-400 outline-none text-sm"
                />
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={handleSend}
                  className="p-2.5 gradient-bg text-white rounded-xl shadow-lg"
                >
